...
Please note that it is not possible to reduce the flow rate as the level increases. 
Reducing an outflow rate in a TRC when the level exceeds a certain threshold is only possible using a DiscreteControl node. 
